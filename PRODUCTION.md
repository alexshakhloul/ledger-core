...
This document covers the architectural decisions, trade-offs,
and production considerations arising directly from my implementation.

## Append-only at scale

### What breaks first

Memory. Everything the replay has seen stays, and nothing is ever released.
...
This is the cheapest option because it defers every row in the table above at once.
The balance index costs less to build, but it covers the query alone.
...
control in the next section. we need to build that control first.
...
that no longer holds, and fees raised on a position that has changed since. This implementation
...
Central Bank at a date. An entry that moves a historical position after the return has been
...
or why, so "who back-dated this, under which authority" has no answer.
...
The point here is not to prevent back-valuing, which is legitimate and necessary. It is to make crossing
...
| Ending                             | Real-world scenario (production use cases)                                        | Mandated behaviour                                                                                                                                                                                                    |
|------------------------------------|-----------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
fixed event stream (like this assessment) neither can happen. Behind any real transport both will, and both would move
money.
...
| Fee reversal/compensation                             | A fee raised on a fact later reversed cannot be undone, so it becomes a dispute queue instead of a ledger entry           |
...
| The event stream is code, not data                    | Changing the stream means changing and redeploying code. Fine for a fixed exercise, not manageable for anything operated  |
| Observability. No metrics and no structured logs      | A skipped fee or a lost hold would leave no trace to find it by                                                           |
Two are worth closing before anything else:
- Event idempotency, because duplicate delivery will happen sooner or later and the failure moves money.
- Authorization expiry, because a permanently pinned hold is the failure a customer notices and calls about.
